...
public interface OriginRequestProvider {
  ...
  static OriginRequestProvider fixedAddress(SocketAddress address) {
    return new OriginRequestProvider() {
      @Override
      public Future<HttpClientRequest> create(ProxyContext proxyContext, HttpClient client) {
        return client.request(new RequestOptions().setServer(address));
      }
    };
  }
  ...
  static OriginRequestProvider selector(Function<ProxyContext, Future<SocketAddress>> selector) {
    return new OriginRequestProvider() {
      @Override
      public Future<HttpClientRequest> create(ProxyContext proxyContext, HttpClient client) {
        return selector.apply(proxyContext).flatMap(server -> client.request(new RequestOptions().setServer(server)));
      }
    };
  }

  Future<HttpClientRequest> create(ProxyContext proxyContext, HttpClient client);
}
