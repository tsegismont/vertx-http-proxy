...
public interface HttpProxy extends Handler<HttpServerRequest> {
   ...
   * @return a reference to this, so the API can be used fluently
   * @deprecated as of Vert.x 5.0, use {@link #origin(OriginRequestProvider)}
   */
  @Deprecated
  @Fluent
  ...
  default HttpProxy originRequestProvider(BiFunction<HttpServerRequest, HttpClient, Future<HttpClientRequest>> provider) {
    return origin(new OriginRequestProvider() {
      @Override
      public Future<HttpClientRequest> create(ProxyContext proxyContext, HttpClient client) {
        return provider.apply(proxyContext.request().proxiedRequest(), client);
      }
    });
  }
